...
async function create(
  ...
  if (0 > overallRating || overallRating > 5)
    throw new Error("Overall rating is invalid");
...
async function rename(id, newWebsite) {
  ...
  if (newWebsite.trim().length == 0)
    throw new Error("Website must not be only spaces");
  if (newWebsite.substr(0, 11) != "http://www.")
    throw new Error("Must enter valid website");
  if (newWebsite.slice(-4) != ".com")
    throw new Error("Must enter valid website");
  if (newWebsite.length < 20) throw new Error("Must enter valid website");
